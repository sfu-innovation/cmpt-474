...
app.get('/assignments/:id', accepts('application/json', 'text/html', 'application/xhtml+xml'));
...
app.all(
	...
	function(req, res, next) {
		...
		EvaluatorRepository.findOne(settings, function(err, result) {
			if (err) return next(err);
			if (result) return res.redirect(result.repository.url+'/'+req.params[0]);
