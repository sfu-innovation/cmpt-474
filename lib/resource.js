
var express = require('express'),
	async = require('async'),
	validate = require('./validate');



module.exports = function(config) {
	
	var store = config.store, model = config.model;

	function parse(props) {
	
		
		return function(req, res, next) {
			var obj = { };

			async.map(model.properties, function(item, callback) {
				
				function done(err, value) { 
					callback(err, { property: item, value: value })
				}
				
				//If all properties are selected or the current
				//property is one of the ones we wish to include
				if (!props || props.indexOf(item.name) !== -1)
					//Fetch its data from the request
					item.getValue(req.body, done);
				//Otherwise
				else
					//Just use the default value
					item.getDefaultValue(done)
			}, function(err, result) {
				if (err) return next(err);
				result.forEach(function(item) {
					obj[item.property.name] = item.value;
				});
				req.object = obj;
				next();
			});
		}
	}

	function validateR(props) {
		var properties = model.properties;

		if (props)
			properties = properties.filter(function(p) {
				return props.indexOf(p.name) !== -1;
			})

		var validator = validate.all(properties.map(function(p) {
			return validate.property(p.name, p.validate)
		}));

		return function(req, res, next) {
			validator(req.object, this, function(err, result, value) {
				if (err) return next(err);
				if (!result) return res.send(400, { error: 'INVALID_REQUEST', details: value });
				return next();
			});
		}
	}

	function put() {
		return function(req, res, next) {
			var entry = this;
			if (!req.object) return next('no object with request');
			store.put(req.object, model, function(err, result) {
				if (err) return next(err);
				//FIXME: Somehow get the mount path and inject it
				//into this?
				res.set('Location', '/'+req.object.id);
				res.send(200, req.object);
			})
		}
	}

	function load(param, props) {
		param = param || 'id';
		return function(req, res, next) {
			store.get(req.params[param], model, function(err, result) {
				if (err) return next(err);
				if (!result) return res.send(404, { error: 'NOT_FOUND' });
				req.object = result;
				return next();
			})
		}
	}

	function get() {
		return function(req, res) {
			if (!req.object) return next('no object with request');
			res.send(200, req.object);
		}
	}

	function del(param) {
		param = param || 'id';
		return function(req, res, next) {
			store.delete(req.params[param], model, function(err, result) {
				if (err) return next(err);
				if (!result) return res.send(404, { error: 'NOT_FOUND' });
				res.status(204)
			})
		}
	}

	function show() {
		return function(req, res, next) {
			if (!req.object) return next('no resource to show');
			res.send(200, req.object);
		}
	}

	return {
		parse: parse,
		validate: validateR,
		get: get,
		put: put,
		delete: del,
		show: show,
		load: load
	}
};