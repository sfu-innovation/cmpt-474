		...
		job.submission.populate('repository', function(err, submission) {
			...
			async.auto({
				...
				'results': [ 'container', 'status', function(next, data) {
					...
					var file = data.source+'/results';
					// FIXME: Figure out how to use something like
					//data.container.copy { "Resource": '/source/results' }
					fs.exists(file, function(exists) {
						if (!exists) return next({ error: 'NO_RESULTS_FILE' });
						byline(fs.createReadStream(file)).on('readable', function() {
							var line, obj;
