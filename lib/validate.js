...
function all(config) {
	...
	return function(input, context, callback) {	
		var results = new Array(config.length);
		function process(i) {
			if (i >= config.length) return callback(undefined, true, results);
			config[i](input, context, function(err, result, value) {
				if (err) return callback(err);
				if (!result) return callback(undefined, false, results);
				results[i] = value;
				return process(i+1);
			}) ;
		}
...
function serial(config) {
	...
	return function(input, context, callback) {	
		function process(i, value) {
			...
			config[i](value, context, function(err, result, value) {
				if (err) return callback(err);
				if (!result) return callback(undefined, false, value);
				return process(i+1, value);
			}) ;
		}
...
function any(config) {
	...
	return function(input, context, callback) {	
		var mismatches = [ ];
		function process(i) {
			if (i >= config.length) return callback(undefined, false, {error: 'NOTHING_VALID', from: mismatches });
			config[i](input, context, function(err, result, value) {
				if (err) return callback(err);
...
function notEmpty() {
	return function(input, context, callback) {
		if (!Array.isArray(input)) return callback(undefined, false, 'NOT_ARRAY');
		if (input.length === 0) return callback(undefined, false, 'EMPTY');
		return callback(undefined, true, input);
	}
}
...
function refersTo(model, opts) {
	...
	return function(input, context, callback) {
		...
		store.get(model, { id: input }, function(err, results) {
			if (err) return callback(err);
